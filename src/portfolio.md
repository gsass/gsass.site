...
### Implmenting user groups
  My first project at Tulipm, and a good onboarding opportunity. User Groups added a lot of changes to our user experience and permissions layers, and got me some familiarity with the Typescript, Go, React, and Redux patterns at Tulip.

### Adding custom user roles with configurable permissions
  A followup to the User Groups effort. Custom roles was similar in implementation to groups, but there were two key differentces. The first was that this project had more security ramiifcations so the team spent more time in the design stage. The second was I had to balance competing deadlines between this work and the Sysadmin Authorization project mentioned later. Scheduling skills are critical to engineering in fast-moving environments, so I'm happy to have got the practice.

### Removing legacy Meteor.js code from the data access and API layer of the app
  A technical debt project, but one with a lot of decoupling and testing work. To ensure this change didn't (obviously or subtly) disrupt the user experience, I had to fully characterize some very old parts of our system. At the end of this effort, a colleague and I ran a retro and generated a plan for better testing practices, so we can remove the remaining Meteor code with more confidence in the next phase of this project.

### Securing production instances with authorization checks for sysadmins (technical lead)
  The first technical effort I led at Tulip. This project was challenging because it was driven by an external stakeholder, on their timeline. As a result I got to do a lot of requirement discovery, expectation setting and documentation work throughout the process of working with internal stakeholders while also liaising with the customer.
  ...
  - Honed coordination skills for very cross-functional efforts. When it comes to Authorization, security, regulatory, commercial, and technical teams all get involved.

### Building APIs for user settings and user online statuses (technical lead)
  These APIs take up the majority of my time now. There's a lot of work involved in launching scalable APIs which can support a variety of client types, and I'm learning a lot about system design.
...
Since some of these projects are still ongoing, I'm waiting to document outcomes.
